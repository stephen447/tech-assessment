describe("Crypto App homepage", () => {
  describe("Crypto App Home Page", () => {
    ...
    it("should allow users to type and search for a cryptocurrency", () => {
      cy.get("input[placeholder='Search cryptocurrency...']").type("Bitcoin");
      cy.get(
        "button[aria-label='Click to search for a cryptocurrency']"
      ).click();

      cy.url().should("include", "/search?query=Bitcoin");
    });
  ...
  describe("Header", () => {
    ...
    it("should contain a title", () => {
      cy.contains("Crypto Sphere").should("be.visible");
    });
